...
namespace BookingWebClient.Controllers
{
    public class BillController : Controller
    {
        ...
        public async Task<IActionResult> BillAdmin(string? sea)
        {
            ...
            if (acc != null)
            {
                if (acc.St == 2)
                {
                    ...
                    if (sea != null)
                    {
                        var list = lists.Where(a => a.Phone.Contains(sea)).ToList();
        ...
        {
            return RedirectToAction("BillAdmin", new { sea = search });
        }
