...
namespace BookingWebClient.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> customers(string? sea)
        {
            var acc = await getUser();
            if (acc != null)
            {
                if (acc.St == 2)
                {
                    ...
                    if (sea != null)
                    {
                        var list = listAccounts.Where(a => a.Phone.Contains(sea)).ToList();
                        if (list.Count > 0)
                        {
                            listAccounts = list;
                        }
        ...
        {
            return RedirectToAction("customers", new { sea = search });
        }
